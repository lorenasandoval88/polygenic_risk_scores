import { fetch23andMeParticipants, fetch23andMeParticipants_fast, get23Txt } from 'https://lorenasandoval88.github.io/personal_genomes_project_sdk/dist/sdk.mjs';
...
let participantLoadMode = 'paged'; // 'paged' | 'all'
let allParticipantsFast = null; // cache for fast-fetch results
	...
	my23FileInput.addEventListener("change", async (event) => {
		...
		try {
			...
			if (typeof get23Txt === "function") {
				// SDK is available - get23Txt returns parsed data directly
				parsed = await get23Txt(file);
				//console.log("Using get23Txt from SDK import", parsed);
				// If get23Txt returns an object with a 'dt' property, use it directly
				if (parsed && parsed.dt) {
					parsed = {
						cols: parsed.cols || [],
						dt: parsed.dt || [],
...
computeV4V5Overlap();
//# sourceMappingURL=displayUsers-Dr40Gx6q.mjs.map
