 ...
 */
function populateDemographicSelect(elementId, field, allLabel) {
	...
	const opts = [`<option value="">${allLabel} (${participants.length})</option>`]
		.concat(values.map(v => `<option value="${escapeHtml(v)}">${escapeHtml(v)} (${counts.get(v)})</option>`));
	if (counts.has('Unknown')) opts.push(`<option value="Unknown">Unknown (${counts.get('Unknown')})</option>`);
	const prev = sel.value;
	sel.innerHTML = opts.join('');
	if ([...sel.options].some(o => o.value === prev)) sel.value = prev;
...
function populateGenderSelect() { populateDemographicSelect('participantsGenderSelect', 'gender', 'All Genders'); }
function populateRaceSelect() { populateDemographicSelect('participantsRaceSelect', 'race', 'All Races'); }
function populateEthnicitySelect() { populateDemographicSelect('participantsEthnicitySelect', 'ethnicity', 'All Ethnicities'); }
...
function applyParticipantFilters() {
	...
	const ethnicity = ethnicitySel?.value ?? '';
	...
	const matchesDemographic = (p, field, expected) => {
		if (!expected) return true;
		const raw = p?.[field];
		const key = (raw == null || raw === '') ? 'Unknown' : String(raw);
		return key === expected;
	};
	if (gender) list = list.filter(p => matchesDemographic(p, 'gender', gender));
	if (race) list = list.filter(p => matchesDemographic(p, 'race', race));
	if (ethnicity) list = list.filter(p => matchesDemographic(p, 'ethnicity', ethnicity));
	...
	if (race) labelParts.push(race);
	if (ethnicity) labelParts.push(ethnicity);
...
window.onParticipantsGenderChange = function onParticipantsGenderChange(v) {
	const sel = document.getElementById('participantsGenderSelect');
	if (sel && v !== undefined) sel.value = v;
	applyParticipantFilters();
};

window.onParticipantsRaceChange = function onParticipantsRaceChange(v) {
	const sel = document.getElementById('participantsRaceSelect');
	if (sel && v !== undefined) sel.value = v;
	applyParticipantFilters();
};

window.onParticipantsEthnicityChange = function onParticipantsEthnicityChange(v) {
	const sel = document.getElementById('participantsEthnicitySelect');
	if (sel && v !== undefined) sel.value = v;
	applyParticipantFilters();
};
		...
		const rowsHtml = pageItems.map((p, i) => {
			...
			return `
				<tr>
					...
					<td>${ageHtml}</td>
					<td>${genderHtml}</td>
					<td title="${raceHtml}">${raceHtml}</td>
					<td title="${ethnicityHtml}">${ethnicityHtml}</td>
...
//# sourceMappingURL=displayUsers-CcL8GmVC.mjs.map
