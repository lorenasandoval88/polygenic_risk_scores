...
// Reflect count on the curated JSON button now that it's loaded
{
	...
	if (jsonBtn) jsonBtn.textContent = `From Curated List with build (${participants.length}) — updated 06-2026`;
}
...
function populateRaceSelect() { populateDemographicSelect('participantsRaceSelect', 'race'); }
function populateEthnicitySelect() { populateDemographicSelect('participantsEthnicitySelect', 'ethnicity'); }
...
function applyParticipantFilters() {
	...
	const filterByDemographic = (arr, field) => {
	...
	filterByDemographic(genders, 'gender');
	filterByDemographic(races, 'race');
	filterByDemographic(ethnicities, 'ethnicity');
...
//# sourceMappingURL=displayUsers-BEN_CWcI.mjs.map
